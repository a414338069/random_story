...
export function normalizeFromPydantic(state: PlayerStatePydantic): NormalizedGameState {
  return {
    ...
    maxHealth: (state as unknown as Record<string, unknown>).max_health as number ?? 100,
    qi: state.qi ?? 0,
    maxQi: (state as unknown as Record<string, unknown>).max_qi as number ?? 0,
...
export function normalizeFromDict(state: GameStateDict): NormalizedGameState {
  return {
    ...
    age: state.age,
    health: (state as unknown as Record<string, unknown>).health as number ?? 100,
    maxHealth: (state as unknown as Record<string, unknown>).max_health as number ?? 100,
    qi: (state as unknown as Record<string, unknown>).qi as number ?? 0,
    maxQi: (state as unknown as Record<string, unknown>).max_qi as number ?? 0,
    lifespan: state.lifespan,
    faction: state.faction,
    spiritStones: state.spirit_stones,
